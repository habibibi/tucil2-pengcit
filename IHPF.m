function outImg = IHPF(img, D0)
% IHPF Melakukan ideal high pass filter melalui ranah frekuensi
%   D0 : frekuensi cutoff
    [N, M, C] = size(img);
    P = 2*N;
    Q = 2*M;
    ...
    for ch = 1:C
        ...
        % Membuat mask filter high-pass ideal dengan cutoff frekuensi D0
        H = double(D <= D0);
        H = fftshift(1-H);
